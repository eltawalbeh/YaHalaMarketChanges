...
const timeout = <T,>(promise: Promise<T>, ms: number) => Promise.race([promise, new Promise<T>((_, reject) => setTimeout(() => reject(new Error("Supabase request timed out. Check the database migration, RLS policies, and network connection.")), ms))])
...
export default function OfferWizard() {
  ...
  const ar = lang === "ar"
  const canPublish = user?.role === "manager" || user?.role === "super_admin"
  ...
  const saveOffer = async (status: "draft" | "published") => {
    setError("")
    if (!user) { setError(ar ? "جلسة الدخول غير متاحة. سجّل الدخول مرة أخرى." : "Your session is not available. Please sign in again."); return }
    if (status === "published" && !canPublish) { setError(ar ? "لا تملك صلاحية نشر العروض." : "You do not have permission to publish offers."); return }
    ...
    try {
      await timeout(offersService.create({
        ...
        created_by: user.id, reviewed_by: status === "published" ? user.id : null, published_at: status === "published" ? new Date().toISOString() : null, expires_at: form.expires_at || null,
      }), 12000)
  ...
  return (
    <DashboardLayout>
      <div className="max-w-3xl">
        ...
        <div className="mb-8 flex items-center gap-2 overflow-x-auto">{STEPS.map((item, itemIndex) => <div key={item} className="flex shrink-0 items-center gap-2"><span className={"flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold " + (itemIndex <= index ? "bg-[var(--primary)] text-white" : "bg-[var(--muted)] text-[var(--muted-foreground)]")}>{itemIndex + 1}</span><span className="text-sm">{ar ? LABELS[item].ar : LABELS[item].en}</span>{itemIndex < STEPS.length - 1 && <span className="h-px w-6 bg-[var(--border)]" />}</div>)}</div>
        ...
        <Card>
          ...
          {step === "review" && <div className="space-y-3 text-sm"><p><strong>{ar ? "العنوان" : "Title"}:</strong> {form.title_ar || form.title}</p><p><strong>{ar ? "الوجهة" : "Destination"}:</strong> {form.city_ar || form.city}, {form.country_ar || form.country}</p><p><strong>{ar ? "السعر" : "Price"}:</strong> {form.price || "0"} {form.currency}</p><p className="text-[var(--muted-foreground)]">{canPublish ? (ar ? "يمكنك حفظه كمسودة أو نشره مباشرة." : "You can save it as a draft or publish it directly.") : (ar ? "سيتم حفظ العرض كمسودة للمراجعة." : "The offer will be saved as a draft for review.")}</p></div>}
        </Card>
        <div className="mt-4 flex flex-wrap justify-between gap-3"><Button variant="secondary" disabled={saving} onClick={() => index === 0 ? navigate(DASHBOARD_ROUTES.offers) : setStep(STEPS[index - 1])}>{ar ? "السابق" : "Back"}</Button>{index < STEPS.length - 1 ? <Button onClick={() => { setError(""); setStep(STEPS[index + 1]) }}>{ar ? "التالي" : "Next"}</Button> : <div className="flex flex-wrap gap-2"><Button variant="secondary" disabled={saving} onClick={() => saveOffer("draft")}>{saving ? (ar ? "جارٍ الحفظ…" : "Saving…") : (ar ? "حفظ كمسودة" : "Save as draft")}</Button>{canPublish && <Button disabled={saving} onClick={() => saveOffer("published")}>{saving ? (ar ? "جارٍ النشر…" : "Publishing…") : (ar ? "حفظ ونشر مباشرة" : "Save & publish")}</Button>}</div>}</div>
      </div>
    </DashboardLayout>
  )
}
